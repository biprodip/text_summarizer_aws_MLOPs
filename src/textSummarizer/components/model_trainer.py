...
class ModelTrainer:
    ...
    def train(self):
        ...
        if device == "cuda":
            ...
            model_pegasus.config.use_cache = False  # required when using gradient checkpointing
        ...
        trainer_args = TrainingArguments(
            output_dir=self.config.root_dir, num_train_epochs=1, warmup_steps=1,
            per_device_train_batch_size=1, per_device_eval_batch_size=1,
            weight_decay=0.01, logging_steps=1,
            eval_steps=1, save_steps=1e6,
            gradient_accumulation_steps=16
        ) 
